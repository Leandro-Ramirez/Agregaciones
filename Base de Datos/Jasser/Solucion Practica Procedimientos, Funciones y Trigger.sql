...
if(@idprov=@IDP)
 begin
  insert into Pedidos values(getdate(),@IDP,0,0,1)
 end
else
 begin
...
after insert
as
update Pedidos set SubTotalP=(select sum(subtp) from inserted),
TotalP=((select sum(subtp) from inserted)*1.15) 
from Pedidos pe, Det_Pedidos dp 
  ...
  begin
   if(@CP=@codp)
	begin
     if(@cpe>0 and @pp>0)
	  begin
	   insert into Det_Pedidos values(@IDP,@CP,@cpe,(@cpe*@pp),@pp)
	  end
	 else
	  begin
 ...
 begin
  if(@idco=@IDC)
   ...
   begin
    if(@idped=@IDP)
	 begin
	  insert into Compras values(@IDC,@FC,@IDP,0,0)
	 end
	else
	 begin
      print 'Pedido no registrado'
...
from inserted),totalc=((select sum(subtcom) from inserted)*1.15) 
from Compras c, Det_Compras dc 
where c.Id_Compras=dc.Id_Compras
...
begin
 if(@CP=@codp)
  begin
   if(@pc>0 and @cc>0)
	begin
	 insert into Det_Compras values(@IDC,@CP, @cc,@pc,(@pc*@cc))
	end
   else
	begin
